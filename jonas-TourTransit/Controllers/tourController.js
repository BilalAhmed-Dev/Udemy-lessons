...
exports.setTourId = async (req, res, next) => {
  const doc = await Tour.find({ _id: req.params.tourId });
  if (doc.length <= 0)
    return next(new AppError('no document with that id were found', 404));
...
exports.getTourStats = catchAsync(async (req, res, next) => {
  const stats = await Tour.aggregate([
    {
      $match: { ratingsAverage: { $gte: +req.query.ratingsAverage } },
    },
    {
      $group: {
